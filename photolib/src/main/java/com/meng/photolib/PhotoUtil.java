...
public class PhotoUtil {
    ...
    private static PhotoUtil sLubanUtil = null;

    public static synchronized PhotoUtil getInstabce() {
        if (sLubanUtil == null) {
            synchronized (PhotoUtil.class) {
                if (sLubanUtil == null) {
                    sLubanUtil = new PhotoUtil();
                }
            }
        }
        return sLubanUtil;
     ...
     * @return
     */
    public PhotoUtil startCompression(final Context context, String path) {
        ...
        Luban.with(context)
                ...
                .setCompressListener(new OnCompressListener() { //设置回调
                    ...
                    public void onSuccess(File file) {
                        PhotoUtil.this.mPath = file.getAbsolutePath();
     ...
     * 裁剪
     * @param context
     * @param tag 0:系统相册 1-n:列表进入
     * @param path 路径
     * @param code resultCode
     * @return
     */
    public PhotoUtil tailoring(Context context,int tag, String path, int code) {
        // 裁剪图片意图
        Intent intent = new Intent("com.android.camera.action.CROP");

        if(tag==0){
            String[] split = path.split(":");
            intent.setDataAndType(Uri.parse("file://"+split[1]), "image/*");
        }else {
